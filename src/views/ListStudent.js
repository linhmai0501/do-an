...
function ListStudent() {
  ...
  const [searchTerm, setSearchTerm] = useState(''); // Trạng thái cho tìm kiếm tên hoặc mã sinh viên
...
body: JSON.stringify({ id: userId }),
        ...
        if (updatedUsers.length < pageSize && currentPage < totalPages) {
          const response = await fetch(`http://localhost/do-an/paginate.php/?page=${currentPage + 1}&size=1`);
          const data = await response.json();
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      {currentUserRole === 'Users' && (
                        <>
                          <Button color="danger" onClick={() => handleDelete(user.id)} style={{ marginLeft: '10px' }}><i className="fa fa-trash" aria-hidden="true"></i></Button>
                          <Button color="info" onClick={() => displayInfo(user.id)} style={{ marginLeft: '10px' }}><i className="fa fa-info" aria-hidden="true"></i></Button>
                        </>
                      )}
                    </td>
                  </tr>
